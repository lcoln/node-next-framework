...
const parse = {
  select ({ select, where, group, order, limit }) {
    select = select ? select : ['*']
    if (!select || !this._table)
      return this._error()
    let base = `select ${select.join(',')} from ${this._table} `
    if (where)
      base += `where ${where} `
    if (group)
      base += `group by ${group.join(',')} `
    if (order) {
  ...
  createTable () {
    let { name, charset, collate, table, comment } = this._orm
    ...
    for (let it of table) {
      ...
      if (!primary)
        primary = it.primary ? `PRIMARY KEY (${it.key})` : ''
  ...
  createDB (db) {
    if (!this._database)
      return this._error('请指定数据库名称')
    return `CREATE DATABASE ${this._database}`
  }
}

function join (v) {
  return v.filter(v => v).join().trim()
}
  ...
  query (sql, database) {
    ...
    if (sql.length) {
      this.getConnection().then(conn => {
        console.log({conn})
        let pool = conn.query({sql, timeout: 100}, (err, res, fields) => {
          console.log({err, res})
          err ? defer.reject(new SqlErr(`Sql Error, sql: ${sql}, Error: ${err}`, sql)) : defer.resolve(res)
        })
